...
START_TEST(find_in_mid) {
  ...
  int find_c = 'W';
  ck_assert_ptr_eq(s21_strchr(str, find_c), strchr(str, find_c));
}
END_TEST
...
START_TEST(find_null_terminator) {
  char str[] = "Hello, World!";
  int find_c = '\0';
  ck_assert_ptr_eq(s21_strchr(str, find_c + 1), strchr(str, find_c + 1));
}
END_TEST
...
START_TEST(find_outoff_string) {
  char str1[20] = "Hello";
  int find_c = 'o';
  ck_assert_ptr_eq(s21_strchr(str1, find_c), strchr(str1, find_c));
}
END_TEST

Suite *suite_s21_strchr(void) {
  Suite *s = suite_create("suite_s21_strchr");
